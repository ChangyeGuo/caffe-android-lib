...
N_JOBS=${N_JOBS:-4}
CAFFE_ROOT=${WD}/caffeOnACL-android
...
cmake -DCMAKE_TOOLCHAIN_FILE="${WD}/android-cmake/android.toolchain.cmake" \
      ...
      -DBUILD_docs=OFF \
      -DUSE_ACL=ON \
      -DACL_INCS=${ANDROID_LIB_ROOT}/acl \
      -DACL_LIBS="${ANDROID_LIB_ROOT}/acl/lib/libarm_compute.so ${ANDROID_LIB_ROOT}/acl/lib/libOpenCL.so" \
      -DUSE_LMDB=ON \
      -DUSE_LIBDNN=OFF \
      -DUSE_LEVELDB=OFF \
      -DUSE_HDF5=OFF \
